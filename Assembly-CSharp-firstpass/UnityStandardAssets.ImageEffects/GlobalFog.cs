...
namespace UnityStandardAssets.ImageEffects
{
	...
	internal class GlobalFog : PostEffectsBase
	{
		...
		private void OnRenderImage(RenderTexture source, RenderTexture destination)
		{
			...
			Vector3 v = transform.forward * nearClipPlane - b + b2;
			float num2 = v.magnitude * farClipPlane / nearClipPlane;
			v.Normalize();
			v *= num2;
			Vector3 v2 = transform.forward * nearClipPlane + b + b2;
			v2.Normalize();
			v2 *= num2;
			Vector3 v3 = transform.forward * nearClipPlane + b - b2;
			v3.Normalize();
			v3 *= num2;
			Vector3 v4 = transform.forward * nearClipPlane - b - b2;
			v4.Normalize();
			v4 *= num2;
			identity.SetRow(0, v);
