...
namespace RetroAesthetics
{
	...
	public class RetroCameraEffect : MonoBehaviour
	{
		...
		{
			_material.SetFloat("_OffsetPosY", UnityEngine.Random.Range(-0.25f * amount, 0.25f * amount));
		...
		public void OnRenderImage(RenderTexture source, RenderTexture destination)
		{
			...
			float float2 = _material.GetFloat("_OffsetPosY");
			if (float2 > 0f)
			{
				_material.SetFloat("_OffsetPosY", float2 - UnityEngine.Random.Range(0f, float2));
			}
			else if (float2 < 0f)
			{
				_material.SetFloat("_OffsetPosY", float2 + UnityEngine.Random.Range(0f, 0f - float2));
			}
			float float3 = _material.GetFloat("_ChromaticAberration");
			if (float3 > chromaticAberration)
			{
				_material.SetFloat("_ChromaticAberration", float3 - 15f * Time.deltaTime);
			}
			else if (randomGlitches && UnityEngine.Random.Range(0, 100 - glitchFrequency) == 0)
			{
