...
public sealed class RagdollInfantry : MonoBehaviour
{
	...
	public void AddExtraMove(Vector3 move)
	{
		if (IsRagdolled)
		{
			...
			{
				rigid.RigidBody.AddForce(a / 100f, ForceMode.VelocityChange);
			}
		}
	}
	...
	{
		if (_state == RagdollState.WaitStablePosition && _hipsTransformRigid.velocity.magnitude < 0.1f)
		{
			GetUp();
		}
		if (_state == RagdollState.Animated)
		{
			FinishedRagdoll = true;
		}
		else
		{
			FinishedRagdoll = false;
		}
	}
	...
	private void ActivateRagdollParts(bool activate)
	{
		foreach (RigidComponent rigid in _rigids)
		{
			if (rigid.RigidBody.GetComponent<Collider>() == null)
			{
				string n = rigid.RigidBody.name + "_ColliderRotator";
				rigid.RigidBody.transform.Find(n).GetComponent<Collider>();
			}
			if (activate)
			{
